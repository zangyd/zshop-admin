...
export default {
  ...
  methods: {
    ...mapActions('zshop/account', [
      'logout'
    ]),
    ...mapActions('zshop/menu', [
      'historyClear'
    ]),
    /**
     * 权限检测
    ...
    getMessageUnread() {
      ...
      getMessageUserUnread(null)
        .then(res => {
          ...
          this.$store.commit('zshop/user/setMessage', res.data.total)
        })
    },
    /**
     * 检测版本更新
    ...
    checkUpdate() {
      let host = process.env.NODE_ENV === 'development'
        ...
        : 'https://api.zshop.cn/api/v1/app_install.html'
      ...
      this.$axios({
        ...
        .then(res => {
          ...
          if (util.compareVersion(data.ver, process.env.VUE_APP_VERSION)) {
            ...
            this.$log.capsule('zshop Admin', `New version ${data.ver}`)
    ...
    async resetMenu() {
      ...
      await this.$store.dispatch('zshop/db/set', {
        dbName: 'database',
       ...
       * 请根据实际需要而自行选择
       */
      this.$store.commit('zshop/page/keepAliveClean')
